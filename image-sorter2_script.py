...
input_folder = '/home/fra/temporaryData/ImageClassifier/images/'
...
class ImageGui:
    ...
    def __init__(self, master, labels, paths):
        ...
        # Select image
        inputCSVFile = pd.read_csv("mainData.csv")
        self.imageList = inputCSVFile["nome"] 
        self.numberOFImages = len(self.imageList)
        ...
        # Add progress label
        progress_string = "%d/%d" % (self.index+1, self.numberOFImages+1)
        ...
        #### added in version 2
        # Add sorting label
        self.sorting_label = tk.Label(frame, text=(self.imageList.loc[self.index]), width=15)
    ...
    def move_prev_image(self):
        ...
        if self.index > 0: 
            ...
            self.sorting_label.configure(text=(self.imageList.loc[self.index]))

            currentPath = os.path.join(input_folder, self.imageList.loc[self.index])
